...
import { getInlinePages, getObjectChanges } from '@/lib/utils';
...
const Table = () => {
  ...
  async function editReport(
    ...
    try {
      ...
      if (previousReportData.marketer_name !== session?.user.provided_name) {
      ...
      if (isRecall) {
        if (isRecallAllowed) {
          const recallCountUrl =
            process.env.NEXT_PUBLIC_BASE_URL +
            '/api/report?action=get-recall-count';
        ...
        const editReportUrl =
          process.env.NEXT_PUBLIC_BASE_URL + '/api/report?action=edit-report';
        const editOptions = {
          method: 'POST',
