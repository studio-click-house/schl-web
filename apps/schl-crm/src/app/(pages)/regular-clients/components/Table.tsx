...
const Table = () => {
  ...
  async function deleteClient(clientData: ReportDataType) {
    try {
  ...
  async function removeClient(
    ...
    try {
      ...
      let url: string =
        process.env.NEXT_PUBLIC_BASE_URL + '/api/report?action=remove-client';
      let options: {} = {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
                ...
                {clients?.items?.map((client, index) => {
                  return (
                    <tr key={String(client._id)}>
                      ...
                      <td>
                        {client.onboard_date &&
                          convertToDDMMYYYY(client.onboard_date)}
                      </td>

                      <td>{client.country}</td>

                      <td className="text-wrap">{client.company_name}</td>
                      <td className="text-wrap">{client.contact_person}</td>
                      <td className="text-wrap">{client.email_address}</td>
